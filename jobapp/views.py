...
from .forms import VideoInterviewForm
...
def home_view(request):
    ...
    if request.headers.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
        job_lists = [job.serialize() for job in page_obj.object_list]
...
def apply_job_view(request, id):
    ...
    if not applicant:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                # Send confirmation email to the applied candidate
                application = Application.objects.get(job=id, applicant=instance)
    ...
    def form_valid(self, form):
        application = Application.objects.create(job=self.job, company=self.job.company, applicant=self.request.user, resume=form.cleaned_data['resume'])
        send_application_confirmation_email(application)
...
@login_required(login_url=reverse_lazy('account:login'))
@user_is_employee
def video_interview(request, id):
    user=request.user
    job = get_object_or_404(Job, id=id)
    applicant = Applicant.objects.filter(user=request.user, job=job).first()
    if not applicant:
        return redirect('jobapp:single-job', id=id)
    if request.method == 'POST':
        form = VideoInterviewForm(request.POST, request.FILES)
        if form.is_valid():
            interview = form.save(commit=False)
            interview.interviewee = applicant
            interview.save()
            return redirect('jobapp:single-job', id=id)
    else:
        form = VideoInterviewForm()
    return render(request, 'video_interview.html', {'job': job, 'form': form})
...
class CustomPasswordResetCompleteView(PasswordResetCompleteView):
    ...
    # Your password reset confirmation view logic here
      return render(request, 'userauth/password_reset_confirm.html')
     
def send_application_confirmation_email(request):
        subject = f"New job application from {application.applicant.name}"
        message = f"{application.applicant.name} has applied for the {application.job.title} position. Here's their resume: {application.resume.url}"
        email_from = settings.EMAIL_HOST_USER
        recipient_list = [application.job.company.email]
